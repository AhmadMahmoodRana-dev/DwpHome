...
const FourthCard = ({ startWeek, endWeek }) => {
  ...
  useEffect(() => {
    ...
    fetch(`https://dwpcare.com.pk/dwp/revenue?STARTWEEK=${startWeek}&ENDWEEK=${endWeek}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
  ...
  return (
    <>
      <div className="fourth-div 2xl:w-[23.5%]  w-[289px] min-h-[650px] h-auto rounded-[6px] pt-5 px-4">
        <h3 className="text-white font-bold text-[14px] 2xl:text-[1.1vw] leading-10 mt-[-22px]">Weekly Revenue</h3>
        <div className="flex gap-3 2xl:gap-1">
          {data.map((item) => (
            <>
              <p className=" text-white font-bold mt-2 text-[45px] leading-9 2xl:text-[3.1vw] 2xl:leading-[2.5vw]">
          ...
          <div className="ml-3">
            <h1 className="font-bold text-white text-[16px] 2xl:text-[1vw] mt-[-30px]">YTD</h1>
            <h3 className="font-thin text-[15px] 2xl:text-[.98vw] text-white leading-[1px] mt-2">
              Revenue
            </h3>
            <h2 className="text-white font-bold text-[20px] mt-2 2xl:text-[1.3vw]">{item.YTD_REVENUE}</h2>
        ...
        {/* ### PROGRESS BAR  */}
        <p className="text-white text-[13px] pb-2 mt-1 2xl:text-[.8vw]">Millions</p>
        {data.map((item) => (
          <>
            <div className="progressBar flex justify-between">
              ...
              <div className="handle-progress">
                <h1 className="text-[10px] 2xl:text-[.6vw] font-semibold text-white text-center mt-[-12px]">
                  Visit <br />
                  Charges
                </h1>
                <div className="text-[12px] 2xl:text-[.7vw] font-semibold text-white">
                  <SemiCircleProgressBar
                    percentage={item.VISIT_PER}
                    showPercentValue
                    stroke={"#000F36"}
                    strokeWidth={7}
                    orientation={"up"}
        ...
        {/* TAble */}
        <div className="px-1 mt-4 pb-3">
          <table className="w-[250px] 2xl:w-[100%]">
            <tr className="text-white">
              <th></th>
              <th className="text-[10px] 2xl:text-[.6vw] font-normal">Parts</th>
              <th className="text-[10px] 2xl:text-[.6vw] font-normal">Services</th>
              <th className="text-[10px] 2xl:text-[.6vw] font-normal">Visits</th>
              <th className="text-[10px] 2xl:text-[.6vw] font-normal">Inst/Corp</th>
            </tr>
            {data2.map((item) => (
              <>
                <tr>
        ...
        {data.map((item) => (
          <>
            <div className="chart flex items-center px-1 mt-[12px]">
              <div className="content leading-5 2xl:leading-[1.3vw] flex flex-col justify-center items-center min-w-[50px] max-w-[50px] 2xl:min-w-[3.3vw] 2xl:max-w-[3.3vw]">
                <h1 className="font-bold text-white text-[14px] 2xl:text-[.9vw]">Center</h1>
            ...
            <div className="chart flex items-center px-1 ">
              <div className="content leading-5 flex flex-col 2xl:leading-[1.3vw] justify-center items-center min-w-[50px] max-w-[50px] 2xl:min-w-[3.3vw] 2xl:max-w-[3.3vw]">
                <h1 className="font-bold text-white text-[14px] 2xl:text-[.9vw]">North</h1>
                <h1 className="text-white font-bold text-[20px] 2xl:text-[1.4vw]">
                  {item.NORTH_REVENUE}
                </h1>
                <p className="font-extralight text-[11px] text-white 2xl:text-[.7vw]">
            ...
            <div className="chart flex items-center px-1 ">
              <div className="content leading-5 flex flex-col 2xl:leading-[1.3vw] justify-center items-center min-w-[50px] max-w-[50px] 2xl:min-w-[3.3vw] 2xl:max-w-[3.3vw]">
                <h1 className="font-bold text-white text-[14px] 2xl:text-[.9vw]">South</h1>
                <h1 className="text-white font-bold text-[20px] 2xl:text-[1.4vw]">
                {item.SOUTH_REVENUE}
                </h1>
                <p className="font-extralight text-[11px] text-white 2xl:text-[.7vw]">
            ...
            {/* <div className="chart flex items-center px-1">
              <div className="content leading-5 flex flex-col justify-center items-center 2xl:leading-[1.3vw] min-w-[50px] max-w-[50px]">
                <h1 className="font-bold text-white text-[14px] 2xl:text-[.9vw]">South</h1>
                <h1 className="text-white font-bold text-[20px] 2xl:text-[1.4vw]">
                  {item.SOUTH_REVENUE}
                </h1>
              ...
              {item.SOUTH_REVENUE_PER >= 0 ? (
                <div className="icons flex flex-col items-center mx-[7px] arrows">
                  <RiTriangleFill className="text-[#148D00] w-[20px] h-[20px] 2xl:w-[1.3vw] 2xl:h-[1.3vw]" />
                  <h1 className="text-[16px] font-bold text-[#148D00] 2xl:text-[.9vw]">
                    +
                    {Math.abs(item.SOUTH_REVENUE_PER).toString().length === 1
                      ? "0" + Math.abs(item.SOUTH_REVENUE_PER)
                      : Math.abs(item.SOUTH_REVENUE_PER)}
                    %
                  </h1>
                ...
                <div className="icons flex flex-col items-center mx-2 arrows">
                  <h1 className="text-[16px] font-bold text-[#BE1A1A] 2xl:text-[.9vw]">
                    -
                    {Math.abs(item.SOUTH_REVENUE_PER).toString().length === 1
                      ? "0" + Math.abs(item.SOUTH_REVENUE_PER)
              ...
              <div className="ml-[9px] 2xl:ml-[.65vw] w-full h-[3.5vw] min-h-[50px]">
              <FourthChart3 chartData={datasouth} />
              </div>
            </div> */}
            
            <div className="flex mt-1 mb-1  bg-slate-300 w-[95%] gap-4 ml-[17px] 2xl:ml-[.6vw] px-2 rounded-[6px]">
              <p className="text-[11px] 2xl:text-[.7vw] text-[#05367d]">Parts</p>
              <p className="text-[11px] 2xl:text-[.7vw] text-[#ce0265]">Services</p>
              <p className="text-[11px] 2xl:text-[.7vw] text-[#148D00]">Visit Charges</p>
           
              <p className="text-[11px] 2xl:text-[.7vw] text-[#ae8104]">Inst/Corp</p>
            </div>
          </>
        ))}
      </div>
